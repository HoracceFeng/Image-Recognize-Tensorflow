...
class VGG19():
    ...
    def network(self, x):
        with tf.variable_scope('VGG16', [x], reuse=self.reuse):
            ...
            x3_4 = conv(x3_3,    {'nlayer':10, 'filter':256,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x3_5 = maxpool(x3_4, {'nlayer':11,             'kernel':[2,2], 'stride':2, 'padding':'same'})
            ## Block-4
            x4_1 = conv(x3_5,    {'nlayer':12,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x4_2 = conv(x4_1,    {'nlayer':13,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x4_3 = conv(x4_2,    {'nlayer':14,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x4_4 = conv(x4_3,    {'nlayer':15,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x4_5 = maxpool(x4_4, {'nlayer':16,             'kernel':[2,2], 'stride':2, 'padding':'same'})
            ## Block-5
            x5_1 = conv(x4_5,    {'nlayer':17,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x5_2 = conv(x5_1,    {'nlayer':18,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x5_3 = conv(x5_2,    {'nlayer':19,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x5_4 = conv(x5_3,    {'nlayer':20,'filter':512,'kernel':[3,3], 'stride':1, 'padding':'same', 'bias':True, 'BN':True, 'activate':True, 'train':self.training})
            x5_5 = maxpool(x5_4, {'nlayer':21,             'kernel':[2,2], 'stride':2, 'padding':'same'})
            ## 3 FC layer
            x_fc1= fully_connected(x5_5,  {'nlayer':22, 'filter':4096,             'activate':True })
            x_fc1= flatten(x_fc1)
            x_fc2= fully_connected(x_fc1, {'nlayer':23, 'filter':4096,             'activate':True })
            x_fc3= fully_connected(x_fc2, {'nlayer':24, 'filter':self.classes_num, 'activate':False})
            ## last-layer: softmax
            # logit= softmax(x_fc3,         {'nlayer':25})
