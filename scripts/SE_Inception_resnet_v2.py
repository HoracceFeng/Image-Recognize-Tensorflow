...
class SE_Inception_resnet_v2():
    ...
    def Build_SEnet(self, input_x):
        with tf.variable_scope('SEnet', [input_x], reuse=self.reuse):
            ...
            input_x = tf.pad(input_x, [[0, 0], [self.size, self.size], [self.size, self.size], [0, 0]])
            # size 32 -> 96
            print ('SEnet input_x :', np.shape(input_x))
            # only cifar10 architecture

            x = self.Stem(input_x, scope='stem')
            print ('SEnet Stem_x :', np.shape(x))
            ...
            print ('SEnet dropout :', np.shape(x))
            x = flatten(x)
            print ('SEnet flatten :', np.shape(x))

            print ("build network FC num-class:", self.classes_num)
            x = Fully_connected(x, units=self.classes_num, layer_name='final_fully_connected')
            print ('SEnet FC :', np.shape(x))
            return x
